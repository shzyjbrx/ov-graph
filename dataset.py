...
class CompositionDataset(tdata.Dataset):
    def __init__(
        ...
        # LLM 邻域节点加载（独立于原 GloVe NEL，供图模块使用）
        # 只做元信息存储，不影响 extra_pairs/attrs/objs
        use_llm_nel = getattr(cfg.MODEL, 'use_llm_nel', False)
        if use_llm_nel:
            llm_nel_dir = getattr(cfg.MODEL, 'llm_nel_dir', '')
            self.llm_nel_dir  = llm_nel_dir
            self.llm_nel_paths = {
                'attr': os.path.join(llm_nel_dir, 'attr_neighbors.json'),
                'obj':  os.path.join(llm_nel_dir, 'obj_neighbors.json'),
                'comp': os.path.join(llm_nel_dir, 'comp_neighbors.json'),
            }
            if is_main_process:
                print(f'[Dataset] LLM NEL 路径已配置: {llm_nel_dir}')
        else:
            self.llm_nel_dir   = None
            self.llm_nel_paths = None
