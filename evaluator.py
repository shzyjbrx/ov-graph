...
class Evaluator:
    ...
    def generate_predictions(self, scores, obj_truth, bias = 0.0, topk = 5): # (Batch, #pairs)
        ...
        mask = self.oracle_obj_mask.to(obj_truth.device)[obj_truth]
    ...
    def score_fast_model(self, scores, obj_truth, bias = 0.0, topk = 5):
        ...
        results = {}
        mask = self.seen_mask.to(scores.device).repeat(scores.shape[0],1) 
        scores[~mask] += bias # Add bias to test pairs
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk = 1):
        ...
        if idx == len(biaslist):
            ...
            sa_so_acc = self.accuracy(scores[sa_so].detach().cpu(), pair_truth[sa_so].cpu(),[1, 3, 5, 10]) 
            ua_so_acc = self.accuracy(scores[ua_so].detach().cpu(), pair_truth[ua_so].cpu(),[1, 3, 5, 10])
            sa_uo_acc = self.accuracy(scores[sa_uo].detach().cpu(), pair_truth[sa_uo].cpu(),[1, 3, 5, 10])
            ua_uo_acc = self.accuracy(scores[ua_uo].detach().cpu(), pair_truth[ua_uo].cpu(),[1, 3, 5, 10])
            sa_so_u_acc = self.accuracy(scores[sa_so_u].detach().cpu(), pair_truth[sa_so_u].cpu(),[1, 3, 5, 10])
            print('Topk [1,3,5]')
