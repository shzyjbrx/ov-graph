...
class OACLIPv3(nn.Module):
    ...
    def __init__(self, dset, cfg):
        ...
        # 2. 构建 img_emb_modules
        if 'vit' in self.clip_type.lower():
            img_emb_modules = [
                # 💡 再次确认这里使用的是 feat_dim
        ...
        # 3. 更新 feat_dim：经过上面的卷积层后，通道数变成了 img_emb_dim (例如 2048)
        # 后续的代码（如 fg_estimator 等）将使用这个新的 feat_dim
